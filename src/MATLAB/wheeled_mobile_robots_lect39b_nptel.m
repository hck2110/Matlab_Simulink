...
K_u=2;K_r=8;
for i=1:length(t)
    eta_d(:,i)=[1*sin(0.1*t(i));1-1*cos(0.1*t(i));0.1*t(i)];
    eta_d_dot=[0.1*cos(0.1*t(i));0.1*sin(0.1*t(i));0.1];
    rho=sqrt((eta_d(1,i)-eta(1,i))^2+(eta_d(2,i)-eta(2,i))^2);
    theta=atan2((eta_d(2,i)-eta(2,i)),(eta_d(1,i)-eta(1,i)));
    if theta<0
        theta=2*pi+theta;
    end
    % if t(i)>=62.8
    ...
    zeta(:,i)=[rho;alpha];
    % if rho<0.1
    %    zeta(2,i)=eta(3,i)-eta_d(3);
    % end
u=K_u*zeta(1,i);r=-K_r*zeta(2,i);
 eta(:,i+1)=eta(:,i)+(1-exp(-1*t(i)))*[cos(eta(3,i)),0;sin(eta(3,i)),0;0,1]*[u;r]*dt;
%zeta(:,i+1)=zeta(:,i)+(1-exp(-1*t(i)))*dt*[-K_u*zeta(1,i)*cos((:,i)*dt;
end
